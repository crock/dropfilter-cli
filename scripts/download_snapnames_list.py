...
def reformat(name):
	...
	with open(f"lists/{service_name}/{name}.txt", 'r') as fx:
		lines = fx.readlines()
		del lines[0]
		for line in lines:
			parts = line.split('\t')
...
for item in all_download_times:
	...
	try:
		...
		if response:
			...
			source_path = os.path.join(f"lists/{service_name}", f"{name}.txt")
			dest_path = os.path.join(f"lists/{service_name}", f"{dl_time}.txt")
